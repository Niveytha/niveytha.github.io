...
var InputManager = function () {
  ...
  function eventIsModifier(event) {
    return (
      ...
      event.keyCode == key.cmd
    );
  }

  function isModifierKeyDown() {
    return (
      self.isKeyDown(key.shift) ||
      self.isKeyDown(key.ctrl) ||
      self.isKeyDown(key.alt) ||
      self.isKeyDown(key.cmd)
    );
  }
  ...
  this.ignoreHeldKeys = function () {
    for (var key in pressed) {
      if (pressed[key]) {
        // only ignore keys that are actually held
        ignored[key] = true;
  ...
  this.ontouchmove = function (event) {
    ...
    if (touchState.isDown && event.changedTouches.length > 0) {
      ...
      if (touchState.curX - touchState.startX <= -touchState.swipeDistance) {
        ...
        touchState.curX - touchState.startX >=
        touchState.swipeDistance
      ) {
        touchState.swipeDirection = SwipeDir.Right;
      } else if (
        touchState.curY - touchState.startY <=
        -touchState.swipeDistance
      ) {
  ...
  this.isKeyDown = function (keyCode) {
    return (
      ...
      (ignored[keyCode] == null || ignored[keyCode] == false)
    );
  };
  ...
  this.anyKeyDown = function () {
    ...
    for (var key in pressed) {
      if (
        pressed[key] &&
        (ignored[key] == null || ignored[key] == false) &&
        !(
          key === key.up ||
          key === key.down ||
          key === key.left ||
...
function renderGame() {
  ...
  var startIndex =
    curGraphicsMode === 0 ? screenBufferId : drawingBuffers.length - 1;
...
function quitGame() {
  ...
  if (isPlayerEmbeddedInEditor) {
    ...
    if (existingTouchTrigger !== null) {
      existingTouchTrigger.removeEventListener(
        "touchstart",
        input.ontouchstart
      );
  ...
  if (buffer.imageData) {
    for (var sy = 0; sy < buffer.scale; sy++) {
      for (var sx = 0; sx < buffer.scale; sx++) {
        var pixelIndex =
          (y * buffer.scale + sy) * buffer.width * buffer.scale * 4 +
          (x * buffer.scale + sx) * 4;
...
function bitsyGetButton(buttonCode) {
  switch (buttonCode) {
    ...
    case 2: // LEFT
      return (
        input.isKeyDown(key.left) || input.isKeyDown(key.a) || input.swipeLeft()
      );
    case 3: // RIGHT
      return (
...
function bitsySetGraphicsMode(mode) {
  ...
  if (curGraphicsMode === 0) {
    screenBuffer.imageData = ctx.createImageData(
      screenBuffer.width * screenBuffer.scale,
      screenBuffer.height * screenBuffer.scale
    );
  } else {
...
function bitsyDrawPixel(paletteIndex, x, y) {
  ...
  var buffer = drawingBuffers[curBufferId];
  buffer.instructions.push({
    type: DrawingInstruction.Pixel,
    id: paletteIndex,
    x: x,
    y: y,
  });
...
function bitsyClear(paletteIndex) {
  ...
  drawingBuffers[curBufferId].instructions.push({
    type: DrawingInstruction.Clear,
    id: paletteIndex,
  });
}
...
var TransitionManager = function () {
  ...
  this.BeginTransition = function (
    ...
    transitionStart = new TransitionInfo(
      startImage,
      startPalette,
      startX,
      startY
  ...
  this.UpdateTransition = function (dt) {
    ...
    if (transitionTime >= minStepTime) {
      ...
      if (transitionEffects[curEffect].paletteEffectFunc) {
        var colors = transitionEffects[curEffect].paletteEffectFunc(
          transitionStart,
          transitionEnd,
          step / maxStep
        );
  ...
  this.RegisterTransitionEffect("fade_w", {
    // TODO : have it linger on full white briefly?
    showPlayerStart: false,
    showPlayerEnd: true,
    stepCount: 6,
    pixelEffectFunc: function (start, end, pixelX, pixelY, delta) {
      return delta < 0.5
        ? start.Image.GetPixel(pixelX, pixelY)
        : end.Image.GetPixel(pixelX, pixelY);
    },
  ...
  function createRoomPixelBuffer(room) {
    ...
    var drawTileInPixelBuffer = function (
      sourceData,
      frameIndex,
      colorIndex,
      tx,
      ...
      for (var y = 0; y < tilesize; y++) {
        for (var x = 0; x < tilesize; x++) {
          var color =
            tileColorStartIndex + (frameData[y][x] === 1 ? colorIndex : 0);
...
var TransitionInfo = function (image, palette, playerX, playerY) {
  ...
  this.PlayerCenter = {
    x: Math.floor(playerX * tilesize + tilesize / 2),
    y: Math.floor(playerY * tilesize + tilesize / 2),
  };
};
...
function FontManager(packagedFontNames) {
  ...
  if (
    packagedFontNames != undefined &&
    packagedFontNames != null &&
    packagedFontNames.length > 0 &&
    Resources != undefined &&
    Resources != null
  ) {
      ...
      for (var i = 0; i < lines.length; i++) {
        ...
        if (!isReadingChar) {
          ...
          if (isReadingCharProperties) {
            var args = line.split(" ");
            if (args[0].indexOf("CHAR_") == 0) {
              // Sub-properties start with "CHAR_"
              if (args[0] == "CHAR_SIZE") {
...
function Script() {
  ...
  var Interpreter = function () {
    ...
    this.Run = function (scriptName, exitHandler, objectContext) {
      ...
      script.Eval(localEnv, function (result) {
        OnScriptReturn(localEnv, exitHandler);
      });
    };
    this.Interpret = function (scriptStr, exitHandler, objectContext) {
    ...
    this.RemoveDialogBlockFormat = function (source) {
      ...
      if (sourceLines[0] === Sym.DialogOpen) {
        ...
        while (i < sourceLines.length && sourceLines[i] != Sym.DialogClose) {
          dialogStr +=
            sourceLines[i] +
            (sourceLines[i + 1] != Sym.DialogClose ? "\n" : "");
          i++;
        }
      } else {
        // single line
  ...
  function pagebreakFunc(environment, parameters, onReturn) {
    environment.GetDialogBuffer().AddPagebreak(function () {
      onReturn(null);
    });
  }
  ...
  function itemFunc(environment, parameters, onReturn) {
    ...
    var curItemCount = player().inventory[itemId]
  ...
  var Environment = function () {
    ...
    this.HasFunction = function (name) {
      return functionMap[name] != undefined;
    };
    ...
    this.HasVariable = function (name) {
      return variableMap[name] != undefined;
    };
    this.GetVariable = function (name) {
    ...
    this.HasOperator = function (sym) {
      return operatorMap[sym] != undefined;
    };
  ...
  var LocalEnvironment = function (parentEnvironment) {
    ...
    this.GetDialogBuffer = function () {
      return parentEnvironment.GetDialogBuffer();
    };

    this.HasFunction = function (name) {
      return parentEnvironment.HasFunction(name);
    };
    this.EvalFunction = function (name, parameters, onReturn, env) {
      if (env == undefined || env == null) {
        env = this;
    ...
    this.HasVariable = function (name) {
      return parentEnvironment.HasVariable(name);
    };
    this.GetVariable = function (name) {
      return parentEnvironment.GetVariable(name);
    };
    this.SetVariable = function (name, value, useHandler) {
      parentEnvironment.SetVariable(name, value, useHandler);
    };
    // this.DeleteVariable // not needed in local environment?

    this.HasOperator = function (sym) {
      return parentEnvironment.HasOperator(sym);
    };
    ...
    this.HasObject = function () {
      return curObject != undefined && curObject != null;
    };
    ...
    this.HasProperty = function (name) {
      if (
        curObject &&
        curObject.property &&
        curObject.property.hasOwnProperty(name)
      ) {
        return true;
      } else {
        return false;
    ...
    this.Serialize = function (depth) {
      ...
      for (var i = 0; i < this.children.length; i++) {
        ...
        var shouldIndentAfterLinebreak =
          lastNode && lastNode.type === "function" && lastNode.name === "br";
  ...
  function isInlineCode(node) {
    return (
      isTextEffectBlock(node) ||
      isUndefinedBlock(node) ||
      isMultilineListBlock(node)
    );
  }
  ...
  var ExpNode = function (operator, left, right) {
    ...
    this.VisitAll = function (visitor, depth) {
      ...
      visitor.Visit(this, depth);
      if (this.left != null) this.left.VisitAll(visitor, depth + 1);
      if (this.right != null) this.right.VisitAll(visitor, depth + 1);
    };
  ...
  var IfNode = function (conditions, results, isSingleLine) {
    ...
    this.Serialize = function (depth) {
      ...
      if (isSingleLine) {
        ...
        str +=
          this.children[0].children[0].Serialize() +
          " ? " +
          this.children[0].children[1].Serialize();
        if (
          this.children.length > 1 &&
          this.children[1].children[0].type === Sym.Else
        ) {
          str +=
            " " + Sym.ElseExp + " " + this.children[1].children[1].Serialize();
        }
      } else {
        str += "\n";
        for (var i = 0; i < this.children.length; i++) {
  ...
  var ConditionPairNode = function (condition, result) {
    ...
    this.Serialize = function (depth) {
      ...
      str +=
        Sym.List +
        " " +
        this.children[0].Serialize(depth) +
        " " +
        Sym.ConditionEnd +
        Sym.Linebreak;
    ...
    this.Parse = function (scriptStr, rootId) {
      ...
      if (state.MatchAhead(Sym.DialogOpen)) {
        ...
        var dialogStr = state.ConsumeBlock(
          Sym.DialogOpen + Sym.Linebreak,
          Sym.Linebreak + Sym.DialogClose
        );
    ...
    function ParseDialog(state) {
      ...
      while (!state.Done()) {
        if (state.MatchAhead(Sym.CodeOpen)) {
          ...
          addCodeNodeToList();
        } else if (state.MatchAhead(Sym.Linebreak)) {
          // process new line
...
*/
    function ParseConditional(state) {
      ...
      function parseConditionalItemLine(state) {
        ...
        while (!state.Done() && !(state.Char() === Sym.Linebreak)) {
          ...
          if (state.Char() === Sym.CodeOpen) {
            lineText += state.ConsumeBlock(
              Sym.CodeOpen,
              Sym.CodeClose,
              true /*includeSymbols*/
            );
          } else {
            if (!encounteredConditionEnd) {
              // skip all characters including & after the condition end
              lineText += state.Char();
            }
            state.Step();
          }
      ...
      function trimLeadingWhitespace(text, trimLength) {
        ...
        textSplit = textSplit.map(function (line) {
      ...
      while (!state.Done()) {
        ...
        if (lineResults.whitespace >= requiredLeadingWhitespace) {
          var trimmedText = trimLeadingWhitespace(
            lineResults.text,
            requiredLeadingWhitespace
          );
      ...
      resultStrings = resultStrings.map(function (result) {
...
*/
    function ParseSequence(state, sequenceType) {
      ...
      itemStrings = itemStrings.map(function (item) {
      ...
      if (
        ...
        if (
          ...
          var right = CreateExpression(
            expStr.substring(setIndex + Sym.Set.length)
          );
    ...
    function IsExpression(str) {
      ...
      var containsAnyExpressionOperators =
        textOutsideCodeBlocks.indexOf(Sym.ConditionEnd) != -1 ||
        textOutsideCodeBlocks.indexOf(Sym.Set) != -1 ||
        Sym.Operators.some(function (opSym) {
          return textOutsideCodeBlocks.indexOf(opSym) != -1;
        });
    ...
    function IsConditionalBlock(state) {
      ...
      peakToFirstConditionSymbol = peakToFirstConditionSymbol.slice(
        peakToFirstListSymbol.length
      );
      var hasNoLinebreakBetweenListAndConditionEnd =
        peakToFirstConditionSymbol.indexOf(Sym.Linebreak) == -1;
...
function Dialog() {
  ...
  var DialogRenderer = function () {
    ...
    this.SetFont = function (f) {
      font = f;
      textboxInfo.height =
        textboxInfo.padding_vert * 3 +
        relativeFontHeight() * 2 +
  ...
  var DialogBuffer = function () {
    ...
    this.AddDrawing = function (drawingId) {
      ...
      if (afterManualPagebreak) {
        ...
        rowLength + drawingChar.spacing <= pixelsPerRow ||
        rowLength <= 0
      ) {
        //stay on same row
        curRowArr.push(drawingChar);
    ...
    this.RemoveTextEffect = function (name) {
      activeTextEffects.splice(
        activeTextEffects.findIndex((test) => test.name === name),
        1
      );
  ...
  var ArabicHandler = function () {
    ...
    var disconnectedCharacters = [
      0x0621, 0x0622, 0x0623, 0x0624, 0x0625, 0x0627, 0x062f, 0x0630, 0x0631,
      0x0632, 0x0648, 0x0649, 0x064b, 0x064c, 0x064d, 0x064e,
    ];
    ...
    function ShapeArabicCharacters(word) {
      ...
      for (var i = 0; i < word.length; i++) {
        ...
        var connectedToPreviousChar =
          i - 1 >= 0 &&
          IsArabicCharacter(word[i - 1]) &&
          !IsDisconnectedCharacter(word[i - 1]);

        var connectedToNextChar =
          i + 1 < word.length &&
          IsArabicCharacter(word[i + 1]) &&
          !IsDisconnectedCharacter(word[i]);
  ...
  var RainbowEffect = function () {
    this.DoEffect = function (char, time) {
      char.color =
        rainbowColorStartIndex +
        Math.floor((time / 100 - char.col * 0.5) % rainbowColorCount);
    };
  };
...
var dialog = {};
var palette = {
  //start off with a default palette
  default: {
    name: "default",
    colors: [
      [0, 0, 0],
...
function load_game(gameData, defaultFontData, startWithTitle) {
  ...
  if (!isPlayerEmbeddedInEditor && defaultFontData) {
    ...
    fontManager.AddResource(
      defaultFontName + fontManager.GetExtension(),
      defaultFontData
    );
  }
...
function updateInput() {
  if (dialogBuffer.IsActive()) {
    ...
    if (
      curPlayerDirection != Direction.None &&
      curPlayerDirection != prevPlayerDirection
    ) {
      movePlayer(curPlayerDirection);
...
function isWall(x, y, roomId) {
  if (roomId == undefined || roomId == null) roomId = curRoom;
...
function parseWorld(file) {
  ...
  while (i < lines.length) {
    ...
    if (i == 0) {
    ...
    } else if (getType(curLine) === "ROOM" || getType(curLine) === "SET") {
      //SET for back compat
      ...
      getType(curLine) === "END" &&
      compatibilityFlags.combineEndingsWithDialog
    ) {
...
function serializeWorld(skipFonts) {
  if (skipFonts === undefined || skipFonts === null) skipFonts = false;
...
function parseDrawingCore(lines, i, drwId) {
  ...
  while (
    !isNaN(parseInt(lines[i].charAt(0))) ||
    lines[i].charAt(0) === ">" ||
    lines[i].charAt(0) === ","
  ) {
    if (lines[i].charAt(0) === ">") {
...
function drawRoom(room, frameIndex) {
  // frameIndex is optional
